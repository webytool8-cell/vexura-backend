function Header({ user, onOpenAuth, onLogout }) {
    ...
    const credits = user?.credits !== undefined ? user.credits : 10;
                    ...
                    {user ? (
                        <>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                >
                                    <div className={`w-7 h-7 rounded-[2px] ${avatars[user.objectData.avatar_idx || 0]} text-[var(--text-main)] flex items-center justify-center text-xs font-mono ring-1 ring-black/20 shadow-inner`}>
                                        {user.objectData.username[0].toUpperCase()}
                                    </div>
                        ...
                        <button onClick={onOpenAuth} className="btn btn-secondary px-6 font-mono font-bold text-xs tracking-wide hover:border-[var(--accent)] hover:text-[var(--accent)]">
                            [ SIGN_IN ]
                        </button>
                    )}
                </div>
                        ...
                        {user ? (
                            <>
                                <div className="flex items-center gap-4 p-4 bg-[var(--bg-panel)] border border-[var(--border-dim)] rounded-[4px]">
                                    <div className={`w-10 h-10 rounded-[2px] ${avatars[user.objectData.avatar_idx || 0]} flex items-center justify-center text-lg font-mono text-white`}>
                                        {user.objectData.username[0].toUpperCase()}
                                    </div>
                                    <div className="flex-1">
                                        <div className="text-sm font-bold text-[var(--text-main)] font-mono">{user.objectData.username}</div>
