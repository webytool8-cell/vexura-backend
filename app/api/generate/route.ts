...
// Fallback vector in case AI fails
...
- Return ONLY JSON, nothing else`,
    ...
    // Render SVG string (optional, can also just return JSON)
    const svgString = renderSVG(vector);

    return new Response(
      JSON.stringify({ vector, svgString, qualityScore }),
      { status: 200, headers }
    );
  } catch (error) {
    ...
    return new Response(
      JSON.stringify({
        ...
        svgString: renderSVG(createSimpleFallback("Fallback Icon")),
        qualityScore: 0.5,
