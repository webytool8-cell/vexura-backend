...
1. Delete those slugs from marketplace via authenticated admin delete endpoint.
2. Regenerate each prompt through `/api/automate/generate` (the real pipeline).
...
- Requires server-side env `MARKETPLACE_ADMIN_TOKEN` to be set (used by `DELETE /api/marketplace/[slug]`).
